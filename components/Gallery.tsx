...
const tabs = [
  {
    ...
    image: 'https://picsum.photos/seed/editor/1200/800'
  },
  {
    id: 'workflow',
    ...
    image: 'https://picsum.photos/seed/workflow/1200/800'
  },
  {
    id: 'queue',
    label: 'Queue Manager',
    ...
    image: 'https://picsum.photos/seed/queue/1200/800'
  }
];
          ...
          <div className="lg:w-1/3 flex flex-col gap-4">
            {tabs.map((tab) => (
              <button
                ...
                className={`flex items-start gap-4 p-6 rounded-xl border text-left transition-all duration-300 ${
                  activeTab.id === tab.id
              ...
              >
                <div className={`mt-1 p-2 rounded-lg ${
                  activeTab.id === tab.id ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-400'
                }`}>
                  {tab.icon}
                </div>
                <div>
                  <h3 className={`font-semibold mb-1 ${
                    activeTab.id === tab.id ? 'text-white' : 'text-slate-300'
                  }`}>
                    {tab.label}
                  </h3>
